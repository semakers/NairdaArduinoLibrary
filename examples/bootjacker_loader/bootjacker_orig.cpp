...
//   5. On restart, bjo_write_page() detects the .noinit state and
...
static uint16_t bjo_magic __attribute__((section(".noinit")));
static uint8_t  bjo_state __attribute__((section(".noinit")));
static uint16_t bjo_addr  __attribute__((section(".noinit")));
static uint8_t  bjo_data[SPM_PAGESIZE] __attribute__((section(".noinit")));

// ═══════════════════════════════════════════════════════════════════════════
void bjo_init(void) {
    if (bjo_magic != BJ_MAGIC) {
        bjo_magic = BJ_MAGIC;
        bjo_state = BJ_IDLE;
    }
}
...
static void bj_jump_fill_write(uint16_t addr) {
    cli();
    memcpy((void *)OPTIBOOT_BUF, bjo_data, SPM_PAGESIZE);
    wdt_enable(WDTO_15MS);
...
uint8_t bjo_write_page(uint16_t addr, const uint8_t *data) {
    ...
    if (bjo_state == BJ_WRITTEN && bjo_addr == addr) {
        bjo_state = BJ_IDLE;
        for (uint16_t i = 0; i < SPM_PAGESIZE; i++) {
            if (pgm_read_byte(addr + i) != bjo_data[i])
                return 1;
        }
        return 0;
    }
    ...
    if (bjo_state == BJ_ERASED && bjo_addr == addr) {
        bjo_state = BJ_WRITTEN;
        bj_jump_fill_write(bjo_addr);   // never returns — WDT resets chip
    }

    // ── Phase 1: Erase (first call) ─────────────────────────────────────
    bjo_addr = addr;
    memcpy(bjo_data, data, SPM_PAGESIZE);
    bjo_state = BJ_ERASED;
